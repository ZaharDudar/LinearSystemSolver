...
int main(){
    ...
    std::ofstream file;
    file.open("./CG-BiCG-CGS-Sym.csv");
    file<<"N_iter,CG_error,CG_time,BiCG_error,BiCG_time,CSG_error,CSG_time\n";
    ...
    for(int NIter = 1; NIter <= 500; NIter+=1){
        ...
        for(int a=0; a < N_TO_AV; a++){
            ...
            answ = CG(A, b, NIter, 1e-7, std::vector<TESTED_TYPE>(MATRIX_SIZE));
